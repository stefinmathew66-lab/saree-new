...
export default function FloatingContact({ onGiftClick }) {
  ...
  return (
    <>
      {/* Lookalike Coupon Gift Widget */}
      {onGiftClick && isWidgetVisible && (
        <div className="floating-gift-widget">
          {/* Close button at top right */}
          <button 
            className="gift-widget-close" 
            onClick={(e) => { 
              e.stopPropagation(); 
              setIsWidgetVisible(false); 
            }}
            aria-label="Dismiss welcome coupon widget"
          >
            <X size={10} />
          </button>

          {/* Circular background with saree image */}
          <div className="gift-widget-circle" onClick={onGiftClick}>
            <img 
              src="images/silk_kanchipuram.webp" 
              alt="Welcome Saree Promo Offer" 
              className="gift-widget-bg-img" 
            />
            
            {/* Coupon badge overlay */}
            <div className="gift-widget-coupon-tag">
              <span className="coupon-tag-header">COUPON</span>
              <span className="coupon-tag-rate">10%</span>
              <span className="coupon-tag-off">OFF</span>
            </div>
          </div>
        </div>
      )}
